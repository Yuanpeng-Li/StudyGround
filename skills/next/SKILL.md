...
5. Pull learner signal from two distinct sources:
   - **Cross-course profile** (style, pace, depth, long-term goals) — skim `$STUDYGROUND_DIR/memory/CLAUDE.md`. Don't expect per-track stuck-points here; this file is intentionally cross-course.
   - **This-track stuck-points** — list `tracks/<current_track>/threads/` and read the 2–3 most recent thread JSONs (by mtime) to see what the learner actually got tangled in. Also re-scan the previous lesson for answered `?>` / `?>>` blocks and any `<!-- feedback:start -->` callouts — those are concrete signals of where they paused or struggled.
