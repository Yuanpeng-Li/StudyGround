...
// We render markdown with raw-HTML enabled because the lesson format relies on
// <details>/<summary>, <!-- answer:* --> markers, and inline tags like <sup>.
// To keep that flexibility without becoming an XSS sink, every md.render and
// md.renderInline output is funnelled through DOMPurify before it is ever
// innerHTML-ed. Lessons are author-written but generated by an LLM, and
// material content / streamed tutor replies are even more attacker-influenceable
// through indirect prompt injection — so we treat all rendered markdown as
// untrusted by default.
//
// `ADD_TAGS: ['details', 'summary']` is defensive: those are HTML5 and
// DOMPurify allows them out of the box, but explicitly listing them documents
// that the lesson format depends on them. KaTeX needs MathML, which
// DOMPurify keeps by default in v3. We also keep `data-*` attributes — the
// merger reads `data-index` / `data-kind` / `data-latex` from rendered output.
const SG_PURIFY_CONFIG = {
  // `details` / `summary` are HTML5 and DOMPurify allows them by default; we
  // list them explicitly so a future config tightening doesn't accidentally
  // break the lesson format. We rely on DOMPurify's defaults to keep `button`
  // (the Ask button rendered from markdown-it custom rules), data-attributes,
  // and MathML (for KaTeX). DOMPurify's default behaviour already strips
  // <script>, on*= handlers, and javascript:/data: URIs in href/src — those
  // are the actual XSS vectors we care about.
  ADD_TAGS: ['details', 'summary'],
  // Defensive: these are not in DOMPurify's default whitelist, but be loud
  // about it for the next reader of this file.
  FORBID_TAGS: ['style', 'iframe', 'object', 'embed', 'link', 'meta', 'form'],
  ALLOW_DATA_ATTR: true,
};
function sgSanitize(html) {
  return DOMPurify.sanitize(html, SG_PURIFY_CONFIG);
}
const _renderRaw = md.render.bind(md);
const _renderInlineRaw = md.renderInline.bind(md);
md.render = (src, env) => sgSanitize(_renderRaw(src, env));
md.renderInline = (src, env) => sgSanitize(_renderInlineRaw(src, env));
...
// In-app replacement for window.confirm(). Reuses the shared #confirm-dialog
// markup in index.html. Esc / backdrop / Cancel → false; primary action → true.
// `message` is rendered with HTML-escaped text and white-space: pre-wrap so
// embedded "\n" become real line breaks without allowing markup injection.
function sgConfirm({ title, message, confirmLabel = 'Delete', destructive = true } = {}) {
  return new Promise((resolve) => {
    ...
    if (!dlg || !titleEl || !msgEl || !okBtn || !cancelBtn) {
      // Fallback if markup is missing — fail safe (no destructive action).
      resolve(false);
      return;
    }
    ...
    const onClose = () => finish(false); // Esc fires 'close' on <dialog>
    const onBackdrop = (ev) => {
      // Clicking the backdrop targets the <dialog> itself rather than its
      // inner content. Treat that as cancel.
    ...
    dlg.showModal();
    // Default focus on Cancel so accidental Enter doesn't fire a destructive action.
...
function mergeQuestionBlocks(root) {
  ...
  //
  // Pending Qs (Ask button shown) stay COLLAPSED so the learner pauses
  // and takes a guess before clicking. Pre-answered Qs (modern lessons)
  // OPEN by default — the skills/ask/SKILL.md contract says the answer
  // "reads as a natural continuation of the lesson"; folding it would
  // hide content the author intends to show inline.
...
document.addEventListener('click', (ev) => {
  ...
  if (action === 'pick-lesson') {
    ...
    if (!slug) return;
    // Reflect the picked lesson in the URL so refresh / share / bookmark
    // restores the same view. hashchange → route() does the load — no need
    // to call loadLesson directly. If we're not in a track context (rare),
    // fall back to loading without touching the URL.
    if (currentTrack) {
      const next = `#/t/${encodeURIComponent(currentTrack)}/${encodeURIComponent(slug)}`;
      if (location.hash !== next) location.hash = next;
      else loadLesson(slug);
    } else {
      loadLesson(slug);
    }
...
es.addEventListener('message', async (ev) => {
  ...
  if (data.type === 'lesson-change') {
    ...
    loadList().then(() => {
      const stillExists = _allLessons.includes(data.file);
      if (!stillExists) {
        // File was deleted / moved (rename event with no replacement). Don't
        // refetch the dead slug — that throws a 404 in the console. If it
        // was the lesson on screen, switch to the first remaining one, or
        // show an empty state.
        if (currentSlug === data.file) {
          const fallback = _allLessons[0];
          if (fallback) loadLesson(fallback);
          else {
            currentSlug = '';
            view.innerHTML = '<p class="hint">That lesson was removed. Pick another or click <b>Next →</b>.</p>';
            lessonTitleBar.textContent = '';
      ...
      else if (!currentSlug) loadLesson(data.file);
    });
  }
...
function parseRoute() {
  ...
  // (long form, kept for backwards compatibility with old links).
  ...
  // #/t/<slug>/<lessonSlug> → reader pinned to a specific lesson (short
  // form — what `pick-lesson` writes into the URL so refresh / bookmark
  // restores the lesson). Avoid matching `intake` or the long-form
  // `lesson/...` prefix.
  const lessonShort = hash.match(/^#\/t\/([^/]+)\/([^/?#]+)\/?$/);
  if (lessonShort) {
    const lessonSlug = decodeURIComponent(lessonShort[2]);
    if (lessonSlug !== 'intake' && lessonSlug !== 'lesson') {
      return { name: 'reader', slug: decodeURIComponent(lessonShort[1]), lesson: lessonSlug };
    }
  }
...
document.addEventListener('click', async (ev) => {
  ...
  if (action === 'open-new-track') {
    ...
    if (!(await sgConfirm({
      title: 'Delete course',
      message: `Delete course "${slug}"?\n\nThis removes the entire tracks/${slug}/ folder (lessons, exercises, materials, threads, curriculum). Irreversible.`,
...
async function buildCmdItems(query) {
  ...
  // Cross-course lesson search on home view — only when the user has typed
  // something, so we don't flood the empty palette with every lesson in
  // every course on first open. One /api/tracks fetch + one /api/lessons
  // per track. Cheap interactively; could move to a bulk endpoint later.
  if (!currentTrack && q) {
    try {
      const tracksR = await fetch('/api/tracks').then((r) => r.json());
      const tracks = tracksR.tracks || [];
      const lessonsPerTrack = await Promise.all(
        tracks.map((t) =>
          fetch(`/api/lessons?track=${encodeURIComponent(t.slug)}`)
            .then((r) => r.json())
            .then((j) => ({ t, lessons: j.lessons || [] }))
            .catch(() => ({ t, lessons: [] })),
        ),
      );
      for (const { t, lessons } of lessonsPerTrack) {
        for (const raw of lessons) {
          const slug = typeof raw === 'string' ? raw : raw.slug;
          if (!slug) continue;
          const lessonName = slug.replace(/^\d+-/, '').replace(/-/g, ' ');
          const label = `${t.title || t.slug} · ${lessonName}`;
          if ((label + ' ' + slug + ' ' + t.slug).toLowerCase().includes(q)) {
            items.push({
              label,
              hint: slug,
              icon: '§',
              run: () => {
                location.hash = `#/t/${encodeURIComponent(t.slug)}/${encodeURIComponent(slug)}`;
              },
              group: 'lessons',
            });
          }
